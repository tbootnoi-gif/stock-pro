// src/App.jsx — STOCK PRO v3 (Multi-Branch Stock แยกสาขา)
...
import {
  ...
  LogOut, Eye, EyeOff, CheckSquare, Square, ArrowLeftRight, Shield
} from "lucide-react";
...
// SHARED UI
// ══════════════════════════════════════════════════════════════
function Badge({type}){const c=TYPE_COLOR[type]||"#6b7280";return<span style={{background:c+"20",color:c,border:`1px solid ${c}40`,fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:9999,whiteSpace:"nowrap"}}>{TYPE_LABEL[type]||type}</span>;}
function StockBadge({qty,min}){const lv=stockLv(qty,min),c=lvColor[lv];return<span style={{background:c+"18",color:c,border:`1px solid ${c}35`,fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:9999}}>{lvLabel[lv]}</span>;}
function Stat({label,value,sub,color,Icon}){return(
...
function Card({title,action,children}){return(
...
function Inp({label,children,style}){return<div style={{display:"flex",flexDirection:"column",gap:5,...style}}>{label&&<label style={{fontSize:12,fontWeight:600,color:"#475569"}}>{label}</label>}{children}</div>;}
...
function Msg({type,text}){if(!text)return null;const[c,bg]=type==="error"?["#ef4444","#fee2e2"]:["#10b981","#dcfce7"];return<div style={{background:bg,color:c,border:`1px solid ${c}30`,borderRadius:8,padding:"10px 14px",fontSize:13,fontWeight:500}}>{text}</div>;}
function Btn({children,onClick,color="#f59e0b",disabled,small,outline}){return(
  <button onClick={onClick} disabled={disabled} style={{display:"flex",alignItems:"center",gap:6,padding:small?"6px 12px":"9px 16px",background:outline?"white":color,color:outline?color:"white",border:`1.5px solid ${outline?color:"transparent"}`,borderRadius:8,cursor:disabled?"not-allowed":"pointer",fontSize:small?12:13,fontWeight:600,fontFamily:"inherit",opacity:disabled?0.6:1,whiteSpace:"nowrap"}}>{children}</button>
);}
...
function LoginScreen({onLogin}){
  const[user,setUser]=useState("");const[pin,setPin]=useState("");const[show,setShow]=useState(false);
    ...
    <div style={{minHeight:"100vh",background:"#0c1929",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"white",borderRadius:20,padding:"40px 36px",width:360,boxShadow:"0 25px 60px rgba(0,0,0,.3)"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <div style={{color:"#94a3b8",fontSize:13,marginTop:4}}>ระบบจัดการสต็อกหลายสาขา</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <Inp label="ชื่อผู้ใช้"><input value={user} onChange={e=>setUser(e.target.value)} style={S} placeholder="username" onKeyDown={e=>e.key==="Enter"&&handle()}/></Inp>
          <Inp label="PIN">
            <div style={{position:"relative"}}>
              ...
              <button onClick={()=>setShow(!show)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8"}}>{show?<EyeOff size={16}/>:<Eye size={16}/>}</button>
            </div>
          </Inp>
          {err&&<Msg type="error" text={err}/>}
          <button onClick={handle} disabled={loading} style={{padding:"11px",background:"#f59e0b",color:"white",border:"none",borderRadius:9,cursor:"pointer",fontSize:14,fontWeight:700,fontFamily:"inherit"}}>{loading?"กำลังเข้าสู่ระบบ...":"เข้าสู่ระบบ"}</button>
        </div>
        <div style={{marginTop:20,padding:"12px",background:"#f8fafc",borderRadius:8,fontSize:11,color:"#94a3b8",textAlign:"center",lineHeight:1.8}}>admin/1234 | manager1/1111 | staff1/0000</div>
      </div>
    </div>
  );
}
...
// DASHBOARD
// ══════════════════════════════════════════════════════════════
function Dashboard({products,movements,alerts,totalValue,setTab,branch}){
  ...
  const chartData=useMemo(()=>{
    const days=[];
    for(let i=6;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const ms=movements.filter(m=>new Date(m.date).toDateString()===d.toDateString());
      days.push({name:fmtDateS(d.toISOString()),"รับเข้า":Math.round(ms.filter(m=>m.type==="STOCK_IN").reduce((s,m)=>s+m.qty*m.cost,0)),"ออก":Math.round(ms.filter(m=>m.type!=="STOCK_IN").reduce((s,m)=>s+m.qty*m.cost,0))});}
    return days;
  },[movements]);
  const wasteBreak=useMemo(()=>{const g={};movements.filter(m=>m.type!=="STOCK_IN").forEach(m=>{g[m.type]=(g[m.type]||0)+m.qty*m.cost;});return Object.entries(g).map(([k,v])=>({name:TYPE_LABEL[k]||k,val:Math.round(v),color:TYPE_COLOR[k]}));},[movements]);
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        <Stat label="มูลค่าสต็อก" value={`฿${fmt(totalValue,0)}`} sub={`${products.length} SKUs · ${branch?.name||"ทุกสาขา"}`} color="#3b82f6" Icon={Package}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:14}}>
        <Card title={`มูลค่าสินค้า เข้า-ออก (7 วัน) — ${branch?.name||""}`}>
          <div style={{padding:"16px 12px 8px"}}>
            <ResponsiveContainer width="100%" height={175}>
              <BarChart data={chartData} barGap={3} barCategoryGap="35%">
                ...
                <Bar dataKey="รับเข้า" fill="#0ea5e9" radius={[4,4,0,0]}/><Bar dataKey="ออก" fill="#f59e0b" radius={[4,4,0,0]}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div style={{padding:"8px 0"}}>
            ...
            :alerts.slice(0,7).map(a=>(
              <div key={a.id} style={{padding:"9px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid #f8fafc"}}>
                <div><div style={{fontSize:12,fontWeight:600,color:"#0f172a"}}>{a.name}</div><div style={{fontSize:11,color:"#94a3b8"}}>คงเหลือ {fmt(a.qty)} {a.unit}</div></div>
          ...
          <div style={{padding:"12px 20px 16px"}}>
            {wasteBreak.map(w=>{const t=wasteBreak.reduce((s,x)=>s+x.val,0)||1;return(
              <div key={w.name} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}><span style={{fontWeight:600,color:w.color}}>{w.name}</span><span>฿{fmt(w.val,0)}</span></div>
                <div style={{height:5,background:"#f1f5f9",borderRadius:9999}}><div style={{height:5,background:w.color,borderRadius:9999,width:`${(w.val/t*100).toFixed(0)}%`}}/></div>
              </div>);})}
          </div>
        </Card>
        <Card title="การเคลื่อนไหวล่าสุด" action={<button onClick={()=>setTab("history")} style={{fontSize:11,color:"#3b82f6",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit"}}>ดูทั้งหมด →</button>}>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{borderBottom:"1px solid #f1f5f9"}}>{["สินค้า","ประเภท","จำนวน","มูลค่า","ผู้ดำเนินการ","เวลา"].map(h=><th key={h} style={{padding:"10px 14px",textAlign:"left",fontSize:11,color:"#94a3b8",fontWeight:700}}>{h}</th>)}</tr></thead>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"flex",gap:10,alignItems:"center"}}>
        <div style={{position:"relative",flex:1,maxWidth:300}}><Search size={14} style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#94a3b8"}}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ค้นหา..." style={{...S,paddingLeft:32}}/></div>
        <select value={cat} onChange={e=>setCat(e.target.value)} style={{...S,width:150}}><option value="ALL">ทุกหมวด</option>{CATS.map(c=><option key={c}>{c}</option>)}</select>
        <Btn onClick={onAdd}><Plus size={14}/>เพิ่มสินค้า</Btn>
      </div>
      <Card>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr style={{background:"#f8fafc",borderBottom:"1px solid #f1f5f9"}}>{["SKU","ชื่อสินค้า","หมวด","คงเหลือ","ขั้นต่ำ","ราคาทุน","มูลค่า","ที่เก็บ","สถานะ",""].map(h=><th key={h} style={{padding:"11px 14px",textAlign:"left",fontSize:11,color:"#94a3b8",fontWeight:700,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
...
function ProductModal({product,suppliers,onSave,onClose,saving}){
  ...
  const[form,setForm]=useState({sku:"",name:"",cat:CATS[0],unit:UNITS[0],cost:"",sell:"",sid:suppliers[0]?.id||"",min:"",loc:"",...product});
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
      <div style={{background:"white",borderRadius:16,width:560,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)"}}>
        ...
        <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          ...
          {isNew&&<div style={{padding:"10px 12px",background:"#e0f2fe",borderRadius:8,fontSize:12,color:"#0369a1"}}>✅ เพิ่มสินค้าใหม่จะสร้างสต็อก 0 ให้ทุกสาขาอัตโนมัติ</div>}
        </div>
        <div style={{padding:"16px 24px",borderTop:"1px solid #f1f5f9",display:"flex",justifyContent:"flex-end",gap:10}}>
          ...
          <Btn onClick={()=>onSave({...form,cost:+form.cost||0,sell:+form.sell||0,min:+form.min||0})} disabled={saving}><Save size={14}/>{saving?"กำลังบันทึก...":"บันทึก"}</Btn>
        </div>
      </div>
    </div>
  );
...
// STOCK IN
// ══════════════════════════════════════════════════════════════
function StockIn({products,form,setForm,onSubmit,msg,loading}){
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  ...
  return(
    <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:16,alignItems:"start"}}>
      <Card title="📥 รับสินค้าเข้าคลัง">
        <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Inp label="ผู้รับ"><input value={form.done_by} onChange={e=>set("done_by",e.target.value)} style={S}/></Inp>
          </div>
      ...
      {sel&&<Card title="ข้อมูลสินค้า"><div style={{padding:"14px 20px"}}>
        {[["คงเหลือ",`${fmt(sel.qty)} ${sel.unit}`],["ขั้นต่ำ",`${fmt(sel.min)} ${sel.unit}`],["ราคาทุนเดิม",`฿${fmt(sel.cost)}`],["หลังรับ",form.qty?`${fmt(sel.qty+(+form.qty||0))} ${sel.unit}`:"-"]].map(([k,v])=>(
...
// ══════════════════════════════════════════════════════════════
// STOCK OUT (พร้อม Transfer ระหว่างสาขา)
// ══════════════════════════════════════════════════════════════
function StockOut({products,branches,branch,form,setForm,onSubmit,msg,loading}){
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  const sel=products.find(p=>p.id===form.pid);
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:16,alignItems:"start"}}>
      <Card title="📤 เบิก / ตัดสินค้าออก">
        <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {OUT_TYPES.map(t=>{const a=form.type===t,i=ti[t];return(
                <button key={t} onClick={()=>set("type",t)} style={{padding:"9px 6px",border:`2px solid ${a?i.c:"#e2e8f0"}`,borderRadius:9,background:a?i.c+"15":"white",cursor:"pointer",fontFamily:"inherit",textAlign:"center"}}>
                  <div style={{fontSize:18}}>{i.e}</div><div style={{fontSize:11,fontWeight:700,color:a?i.c:"#64748b"}}>{TYPE_LABEL[t]}</div>
                </button>);})}
            </div>
          </Inp>
          ...
          {/* Transfer: เลือกสาขาปลายทาง */}
          {form.type==="TRANSFER"&&(
            <Inp label="โอนไปสาขา *">
              <select value={form.target_branch_id||""} onChange={e=>set("target_branch_id",e.target.value)} style={{...S,borderColor:"#8b5cf6"}}>
                <option value="">-- เลือกสาขาปลายทาง --</option>
                {otherBranches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
              {form.target_branch_id&&<div style={{marginTop:6,fontSize:11,color:"#8b5cf6",fontWeight:600}}>🔄 ระบบจะตัดสต็อกจากสาขานี้ และเพิ่มให้สาขาปลายทางอัตโนมัติ</div>}
            </Inp>
          )}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <Msg type={msg?.startsWith("✓")?"success":"error"} text={msg}/>
          <Btn onClick={onSubmit} disabled={loading} color={form.type==="WASTE"||form.type==="SPOIL"?"#ef4444":form.type==="TRANSFER"?"#8b5cf6":"#f59e0b"}>
            {form.type==="TRANSFER"?<ArrowLeftRight size={16}/>:<ArrowUpCircle size={16}/>}{loading?"กำลังบันทึก...":form.type==="TRANSFER"?"ยืนยันโอนสต็อก":"ยืนยันเบิกออก"}
          </Btn>
        </div>
      </Card>
      {sel&&<Card title="ข้อมูลสินค้า"><div style={{padding:"14px 20px"}}>
...
function StockCount({products,user,branch,onAdjust}){
  const[counts,setCounts]=useState([]);const[activeCount,setActiveCount]=useState(null);const[items,setItems]=useState([]);
  const[loading,setLoading]=useState(false);const[msg,setMsg]=useState("");
  const[newTitle,setNewTitle]=useState(`นับสต็อก ${new Date().toLocaleDateString("th-TH",{month:"long",year:"numeric"})}`);
  const loadCounts=async()=>{const{data}=await supabase.from("stock_counts").select("*").order("created_at",{ascending:false}).limit(20);setCounts(data||[]);};
  ...
  const createCount=async()=>{
    ...
    setCounts(c=>[cnt,...c]);setActiveCount(cnt);setItems(its);setLoading(false);
  };
  const openCount=async(cnt)=>{setActiveCount(cnt);const{data}=await supabase.from("stock_count_items").select("*").eq("count_id",cnt.id).order("pname");setItems(data||[]);};
  const updateItem=async(id,val)=>{const v=val===""?null:+val;setItems(is=>is.map(i=>i.id===id?{...i,actual_qty:v}:i));await supabase.from("stock_count_items").update({actual_qty:v}).eq("id",id);};
  const submitCount=async()=>{setLoading(true);await supabase.from("stock_counts").update({status:"completed"}).eq("id",activeCount.id);setActiveCount(c=>({...c,status:"completed"}));setCounts(cs=>cs.map(c=>c.id===activeCount.id?{...c,status:"completed"}:c));setMsg("✓ ส่งรอบนับเรียบร้อย");setLoading(false);};
  const approveCount=async()=>{
    ...
    for(const it of diffs){await onAdjust(it.pid,it.pname,it.unit,it.cost,it.actual_qty-it.system_qty,`อนุมัติรอบนับ: ${activeCount.title}`);}
    ...
    setActiveCount(c=>({...c,status:"approved"}));setCounts(cs=>cs.map(c=>c.id===activeCount.id?{...c,status:"approved"}:c));
    setMsg(`✓ อนุมัติแล้ว ปรับสต็อก ${diffs.length} รายการ`);setLoading(false);
  };
  const sc={counting:"#3b82f6",completed:"#f59e0b",approved:"#10b981"};
  const sl={counting:"กำลังนับ",completed:"รอนุมัติ",approved:"อนุมัติแล้ว"};
  ...
  if(activeCount)return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1,fontWeight:700,fontSize:16}}>{activeCount.title}</div>
        <span style={{background:sc[activeCount.status]+"20",color:sc[activeCount.status],border:`1px solid ${sc[activeCount.status]}40`,fontSize:12,fontWeight:700,padding:"4px 12px",borderRadius:9999}}>{sl[activeCount.status]}</span>
      </div>
      <Msg type={msg?.startsWith("✓")?"success":"error"} text={msg}/>
      <Card>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr style={{background:"#f8fafc"}}>{["SKU","สินค้า","ระบบ","นับจริง","ส่วนต่าง","หมายเหตุ"].map(h=><th key={h} style={{padding:"11px 14px",textAlign:"left",fontSize:11,color:"#94a3b8",fontWeight:700}}>{h}</th>)}</tr></thead>
            <tbody>{items.map(it=>{const d=diff(it);return(
              <tr key={it.id} style={{borderBottom:"1px solid #f8fafc",background:d!==null&&d!==0?"#fffbeb":"transparent"}}>
                ...
                <td style={{padding:"10px 14px"}}>
                  <input type="number" value={it.actual_qty??""} onChange={e=>updateItem(it.id,e.target.value)} disabled={activeCount.status==="approved"} style={{...S,width:100,border:`1.5px solid ${d!==null&&d!==0?"#f59e0b":"#e2e8f0"}`}} placeholder="-"/>
                </td>
                <td style={{padding:"10px 14px",fontSize:13,fontWeight:700,color:d===null?"#94a3b8":d>0?"#10b981":d<0?"#ef4444":"#64748b"}}>{d===null?"-":d>0?`+${fmt(d)}`:fmt(d)}</td>
                <td style={{padding:"10px 14px"}}><input value={it.note||""} onChange={async e=>{setItems(is=>is.map(i=>i.id===it.id?{...i,note:e.target.value}:i));await supabase.from("stock_count_items").update({note:e.target.value}).eq("id",it.id);}} style={{...S,fontSize:12}} disabled={activeCount.status==="approved"}/></td>
              </tr>);})}</tbody>
          </table>
        </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <Card title="สร้างรอบนับสต็อกใหม่">
        <div style={{padding:"16px 20px",display:"flex",gap:10}}>
          <input value={newTitle} onChange={e=>setNewTitle(e.target.value)} style={{...S,flex:1}}/>
          <Btn onClick={createCount} disabled={loading}><Plus size={14}/>สร้างรอบนับ</Btn>
        </div>
      </Card>
      <Card title="ประวัติรอบนับ">
        {counts.length===0?<div style={{padding:"40px",textAlign:"center",color:"#94a3b8"}}>ยังไม่มีรอบนับ</div>
        :counts.map(c=>(
          <div key={c.id} style={{padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid #f8fafc",cursor:"pointer"}} onClick={()=>openCount(c)} onMouseEnter={e=>e.currentTarget.style.background="#f8fafc"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            <div><div style={{fontWeight:600}}>{c.title}</div><div style={{fontSize:12,color:"#94a3b8",marginTop:2}}>นับโดย: {c.counted_by} | {fmtDate(c.created_at)}</div></div>
            <span style={{background:sc[c.status]+"20",color:sc[c.status],border:`1px solid ${sc[c.status]}40`,fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:9999}}>{sl[c.status]}</span>
          </div>))}
      </Card>
    </div>
  );
...
// ══════════════════════════════════════════════════════════════
// HISTORY / ALERTS / SUPPLIERS / REPORTS (compact)
// ══════════════════════════════════════════════════════════════
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{["ALL","STOCK_IN",...OUT_TYPES].map(t=>(
          <button key={t} onClick={()=>setTf(t)} style={{padding:"7px 12px",border:`1.5px solid ${tf===t?(TYPE_COLOR[t]||"#f59e0b"):"#e2e8f0"}`,borderRadius:8,background:tf===t?(TYPE_COLOR[t]||"#f59e0b")+"15":"white",color:tf===t?(TYPE_COLOR[t]||"#f59e0b"):"#64748b",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"inherit"}}>
            {t==="ALL"?"ทั้งหมด":TYPE_LABEL[t]}</button>))}</div>
      </div>
      <Card>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr style={{background:"#f8fafc"}}>{["วันที่","สินค้า","ประเภท","จำนวน","มูลค่า","เอกสาร","ผู้ดำเนินการ","หมายเหตุ"].map(h=><th key={h} style={{padding:"11px 14px",textAlign:"left",fontSize:11,color:"#94a3b8",fontWeight:700,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
...
function AlertsTab({alerts,setTab}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      {out.length>0&&<Card title={`🔴 หมดสต็อก (${out.length})`}>{out.map(a=>(
      ...
      {low.length>0&&<Card title={`🟠 ใกล้หมด (${low.length})`}>{low.map(a=>{const p=Math.min(Math.round(a.qty/a.min*100),100);return(
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:1,padding:1,background:"#f1f5f9"}}>
        {suppliers.map(s=>{const ps=products.filter(p=>p.sid===s.id);return(
          <div key={s.id} style={{background:"white",padding:"20px 22px"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><div><div style={{fontWeight:700,fontSize:15}}>{s.name}</div><div style={{fontSize:12,color:"#94a3b8",marginTop:2}}>{s.contact}</div></div><span style={{background:"#dbeafe",color:"#1d4ed8",fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:9999}}>{s.id}</span></div>
...
function Reports({products,movements,branch}){
  ...
  const selP=products.filter(p=>sel.has(p.id));
  const filtMovs=movements.filter(m=>sel.has(m.pid)&&new Date(m.date)>=new Date(dateFrom)&&new Date(m.date)<=new Date(dateTo+"T23:59:59"));
  const totalVal=selP.reduce((s,p)=>s+p.qty*p.cost,0);
  const doPrint=()=>{
    const w=window.open("","_blank");
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>STOCK PRO รายงาน</title><style>@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Thai:wght@400;600;700&display=swap');body{font-family:'Noto Sans Thai',sans-serif;font-size:12px;color:#0f172a;margin:0;padding:20px;}table{width:100%;border-collapse:collapse;margin-top:12px;}th{background:#f1f5f9;padding:8px 10px;text-align:left;font-size:11px;color:#475569;border:1px solid #e2e8f0;}td{padding:8px 10px;border:1px solid #e2e8f0;font-size:12px;}tr:nth-child(even) td{background:#f8fafc;}.header{display:flex;justify-content:space-between;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid #0f172a;}.total{margin-top:12px;padding:12px;background:#f1f5f9;border-radius:8px;font-weight:700;}</style></head><body>${printRef.current.innerHTML}</body></html>`);
    w.document.close();w.focus();setTimeout(()=>w.print(),400);
  };
  const rTypes=[{id:"stock",label:"📦 รายงานสต็อก"},{id:"movement",label:"📋 รายงานการเคลื่อนไหว"},{id:"lowstock",label:"⚠️ ของใกล้หมด"}];
  return(
    <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:16,alignItems:"start"}}>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <Card title="ประเภทรายงาน">
          <div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:6}}>
            {rTypes.map(r=><button key={r.id} onClick={()=>setRType(r.id)} style={{padding:"10px 12px",border:`2px solid ${rType===r.id?"#f59e0b":"#e2e8f0"}`,borderRadius:8,background:rType===r.id?"#fff7ed":"white",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:rType===r.id?700:400,color:rType===r.id?"#f59e0b":"#334155",textAlign:"left"}}>{r.label}</button>)}
          </div>
        </Card>
        {rType==="movement"&&<Card title="ช่วงวันที่"><div style={{padding:"12px 16px",display:"flex",flexDirection:"column",gap:8}}><Inp label="จาก"><input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={S}/></Inp><Inp label="ถึง"><input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} style={S}/></Inp></div></Card>}
        <Card title={`เลือกสินค้า (${sel.size}/${products.length})`}>
          <div style={{padding:"8px 12px"}}>
            <button onClick={toggleAll} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#3b82f6",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",padding:"4px 0",marginBottom:6}}>{sel.size===products.length?<CheckSquare size={14}/>:<Square size={14}/>}{sel.size===products.length?"ยกเลิกทั้งหมด":"เลือกทั้งหมด"}</button>
            <div style={{maxHeight:260,overflowY:"auto",display:"flex",flexDirection:"column",gap:3}}>
              {products.map(p=><label key={p.id} style={{display:"flex",alignItems:"center",gap:8,fontSize:12,cursor:"pointer",padding:"4px 2px"}}>
                <input type="checkbox" checked={sel.has(p.id)} onChange={()=>toggle(p.id)} style={{accentColor:"#f59e0b"}}/><span style={{flex:1}}>{p.name}</span><span style={{color:"#94a3b8",fontSize:11}}>{fmt(p.qty)}</span>
              </label>)}
            </div>
          </div>
        </Card>
      ...
      <Card title="ตัวอย่างรายงาน">
        <div style={{padding:"20px 24px"}}>
          <div ref={printRef}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:20,paddingBottom:16,borderBottom:"2px solid #0f172a"}}>
              <div><div style={{fontWeight:800,fontSize:18}}>📦 STOCK PRO</div><div style={{color:"#64748b",fontSize:12}}>{rTypes.find(r=>r.id===rType)?.label.replace(/📦|📋|⚠️/g,"").trim()}</div>{branch&&<div style={{color:"#64748b",fontSize:12}}>สาขา: {branch.name}</div>}</div>
              <div style={{textAlign:"right",fontSize:12,color:"#64748b"}}><div>พิมพ์: {fmtFull(new Date().toISOString())}</div>{rType==="movement"&&<div>ช่วง: {dateFrom} ถึง {dateTo}</div>}<div>สินค้า: {sel.size} รายการ</div></div>
            </div>
            {rType==="stock"&&<><table><thead><tr>{["SKU","ชื่อ","หมวด","คงเหลือ","หน่วย","ขั้นต่ำ","ราคาทุน","มูลค่า","ที่เก็บ","สถานะ"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{selP.map(p=><tr key={p.id}><td style={{fontFamily:"monospace",fontSize:11}}>{p.sku}</td><td style={{fontWeight:600}}>{p.name}</td><td>{p.cat}</td><td style={{fontWeight:700,color:p.qty<p.min?"#ef4444":"#0f172a"}}>{fmt(p.qty)}</td><td>{p.unit}</td><td>{fmt(p.min)}</td><td>฿{fmt(p.cost)}</td><td style={{color:"#3b82f6",fontWeight:600}}>฿{fmt(p.qty*p.cost,0)}</td><td>{p.loc||"-"}</td><td><span style={{background:p.qty<=0?"#fee2e2":p.qty<p.min?"#ffedd5":"#dcfce7",color:p.qty<=0?"#ef4444":p.qty<p.min?"#f97316":"#10b981",padding:"2px 8px",borderRadius:9999,fontSize:10,fontWeight:700}}>{p.qty<=0?"หมด":p.qty<p.min?"ใกล้หมด":"ปกติ"}</span></td></tr>)}</tbody></table><div style={{marginTop:12,padding:"12px",background:"#f1f5f9",borderRadius:8,fontWeight:700}}>มูลค่ารวม: ฿{fmt(totalVal,0)} | {selP.length} SKUs</div></>}
            {rType==="movement"&&<><table><thead><tr>{["วันที่","สินค้า","ประเภท","จำนวน","หน่วย","มูลค่า","ผู้ดำเนินการ","เอกสาร"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{filtMovs.map(m=><tr key={m.id}><td style={{whiteSpace:"nowrap",fontSize:11}}>{fmtDate(m.date)}</td><td style={{fontWeight:600}}>{m.pname}</td><td><span style={{background:TYPE_COLOR[m.type]+"20",color:TYPE_COLOR[m.type],padding:"2px 6px",borderRadius:9999,fontSize:10,fontWeight:700}}>{TYPE_LABEL[m.type]}</span></td><td style={{color:m.type==="STOCK_IN"?"#0ea5e9":"#f59e0b",fontWeight:700}}>{m.type==="STOCK_IN"?"+":"-"}{fmt(m.qty)}</td><td>{m.unit}</td><td>฿{fmt(m.qty*m.cost,0)}</td><td>{m.done_by}</td><td style={{fontFamily:"monospace",fontSize:11}}>{m.ref||"-"}</td></tr>)}</tbody></table><div style={{marginTop:12,padding:"12px",background:"#f1f5f9",borderRadius:8,fontWeight:700}}>รายการ: {filtMovs.length} | รับเข้า: ฿{fmt(filtMovs.filter(m=>m.type==="STOCK_IN").reduce((s,m)=>s+m.qty*m.cost,0),0)} | ออก: ฿{fmt(filtMovs.filter(m=>m.type!=="STOCK_IN").reduce((s,m)=>s+m.qty*m.cost,0),0)}</div></>}
            {rType==="lowstock"&&<><table><thead><tr>{["SKU","สินค้า","คงเหลือ","ขั้นต่ำ","ขาดอีก","มูลค่าที่ต้องสั่ง","สถานะ"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{selP.filter(p=>p.qty<p.min).map(p=><tr key={p.id}><td style={{fontFamily:"monospace",fontSize:11}}>{p.sku}</td><td style={{fontWeight:600}}>{p.name}</td><td style={{color:p.qty<=0?"#ef4444":"#f97316",fontWeight:700}}>{fmt(p.qty)} {p.unit}</td><td>{fmt(p.min)} {p.unit}</td><td style={{color:"#ef4444",fontWeight:700}}>{fmt(Math.max(0,p.min-p.qty))} {p.unit}</td><td style={{color:"#ef4444",fontWeight:600}}>฿{fmt(Math.max(0,p.min-p.qty)*p.cost,0)}</td><td><span style={{background:p.qty<=0?"#fee2e2":"#ffedd5",color:p.qty<=0?"#ef4444":"#f97316",padding:"2px 8px",borderRadius:9999,fontSize:10,fontWeight:700}}>{p.qty<=0?"หมดแล้ว":"ใกล้หมด"}</span></td></tr>)}</tbody></table><div style={{marginTop:12,padding:"12px",background:"#fee2e2",borderRadius:8,fontWeight:700,color:"#991b1b"}}>ต้องสั่งซื้อ {selP.filter(p=>p.qty<p.min).length} รายการ | มูลค่า: ฿{fmt(selP.filter(p=>p.qty<p.min).reduce((s,p)=>s+Math.max(0,p.min-p.qty)*p.cost,0),0)}</div></>}
          </div>
        </div>
      </Card>
    </div>
...
// ══════════════════════════════════════════════════════════════
// BRANCHES (สร้างสาขา + auto-init branch_stock)
// ══════════════════════════════════════════════════════════════
function BranchesMgmt({products,onCreateBranch}){
  const[branches,setBranches]=useState([]);
  const[form,setForm]=useState({name:"",address:"",phone:""});
  const[msg,setMsg]=useState("");const[loading,setLoading]=useState(false);
  useEffect(()=>{supabase.from("branches").select("*").order("name").then(({data})=>setBranches(data||[]));},[]);
  const save=async()=>{
    if(!form.name){setMsg("กรุณากรอกชื่อสาขา");return;}
    setLoading(true);
    const b={...form,id:"B"+UID(),active:true};
    await supabase.from("branches").insert(b);
    // Auto-create branch_stock entries (qty=0) for all existing products
    if(products.length>0){
      const entries=products.map(p=>({id:"BS"+UID(),branch_id:b.id,pid:p.id,qty:0,min:p.min||0,updated_at:new Date().toISOString()}));
      await supabase.from("branch_stock").insert(entries);
    }
    setBranches(bs=>[...bs,b]);
    setForm({name:"",address:"",phone:""});
    setMsg(`✓ สร้างสาขา "${b.name}" เรียบร้อย สร้างสต็อก 0 สำหรับสินค้า ${products.length} รายการแล้ว`);
    if(onCreateBranch)onCreateBranch(b);
    setLoading(false);
    setTimeout(()=>setMsg(""),4000);
  };
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <Card title="➕ สร้างสาขาใหม่">
        <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 2fr 1fr",gap:12}}>
            <Inp label="ชื่อสาขา *"><input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} style={S} placeholder="สาขา 3"/></Inp>
            <Inp label="ที่อยู่"><input value={form.address} onChange={e=>setForm(f=>({...f,address:e.target.value}))} style={S} placeholder="123 ถ.xxx กทม."/></Inp>
            <Inp label="โทร"><input value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))} style={S} placeholder="02-xxx-xxxx"/></Inp>
          </div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <Btn onClick={save} disabled={loading}><Plus size={14}/>{loading?"กำลังสร้าง...":"สร้างสาขา"}</Btn>
            <span style={{fontSize:12,color:"#64748b"}}>✅ สร้างสต็อกเริ่มต้น 0 สำหรับสินค้าทุกรายการอัตโนมัติ</span>
          </div>
          {msg&&<Msg type={msg.startsWith("✓")?"success":"error"} text={msg}/>}
        </div>
      </Card>
      <Card title={`สาขาทั้งหมด (${branches.length} สาขา)`}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:1,padding:1,background:"#f1f5f9"}}>
          {branches.map(b=>(
            <div key={b.id} style={{background:"white",padding:"20px 22px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <div style={{fontWeight:700,fontSize:15}}>{b.name}</div>
                <span style={{background:"#dbeafe",color:"#1d4ed8",fontSize:11,fontWeight:700,padding:"2px 10px",borderRadius:9999}}>{b.id}</span>
              </div>
              <div style={{fontSize:12,color:"#64748b",marginTop:4}}>📍 {b.address||"-"}</div>
              <div style={{fontSize:12,color:"#64748b",marginTop:4}}>📞 {b.phone||"-"}</div>
              <div style={{marginTop:8}}><span style={{background:b.active?"#dcfce7":"#f1f5f9",color:b.active?"#10b981":"#94a3b8",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:9999}}>{b.active?"เปิดใช้งาน":"ปิด"}</span></div>
            </div>))}
        </div>
      </Card>
    </div>
...
function EmployeesMgmt({branches}){
  const[emps,setEmps]=useState([]);const[form,setForm]=useState({name:"",username:"",pin:"",role:"staff",branch_id:branches[0]?.id||""});const[msg,setMsg]=useState("");
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  useEffect(()=>{supabase.from("employees").select("*").order("name").then(({data})=>setEmps(data||[]));},[branches]);
  const save=async()=>{
    if(!form.name||!form.username||!form.pin){setMsg("กรุณากรอกข้อมูลให้ครบ");return;}
  ...
  const toggleActive=async(id,cur)=>{await supabase.from("employees").update({active:!cur}).eq("id",id);setEmps(es=>es.map(e=>e.id===id?{...e,active:!cur}:e));};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <Card title="เพิ่มพนักงานใหม่">
        <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
            <Inp label="ชื่อ *"><input value={form.name} onChange={e=>set("name",e.target.value)} style={S}/></Inp>
            <Inp label="Username *"><input value={form.username} onChange={e=>set("username",e.target.value)} style={S}/></Inp>
            <Inp label="PIN *"><input type="password" value={form.pin} onChange={e=>set("pin",e.target.value)} style={S} maxLength={6}/></Inp>
          ...
          <div style={{display:"flex",gap:12,alignItems:"flex-end"}}>
            <Inp label="ตำแหน่ง"><div style={{display:"flex",gap:8}}>{["staff","manager","admin"].map(r=><button key={r} onClick={()=>set("role",r)} style={{padding:"8px 14px",border:`2px solid ${form.role===r?ROLE_COLOR[r]:"#e2e8f0"}`,borderRadius:8,background:form.role===r?ROLE_COLOR[r]+"15":"white",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700,color:form.role===r?ROLE_COLOR[r]:"#64748b"}}>{ROLE_TH[r]}</button>)}</div></Inp>
      ...
      <Card title={`พนักงานทั้งหมด (${emps.length} คน)`}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{emps.map(e=>{const br=branches.find(b=>b.id===e.branch_id);return(
              <tr key={e.id} style={{borderBottom:"1px solid #f8fafc"}}>
                ...
                <td style={{padding:"11px 14px",fontSize:12,color:"#64748b"}}>{br?.name||e.branch_id||"-"}</td>
...
export default function StockApp(){
  ...
  const[soForm,setSoForm]=useState({pid:"",qty:"",type:"SALE",ref:"",note:"",done_by:"",date:now,target_branch_id:""});
  const[siMsg,setSiMsg]=useState("");const[soMsg,setSoMsg]=useState("");

  // ── Load data ──────────────────────────────────────────────
  const loadAll=async(bid)=>{
    setLoading(true);
    try{
      const bId=bid!==undefined?bid:branch?.id;
      const[{data:prods},{data:bs},{data:movs},{data:sups},{data:brs}]=await Promise.all([
        supabase.from("products").select("*").order("name"),
        bId?supabase.from("branch_stock").select("*").eq("branch_id",bId):{data:[]},
        bId?supabase.from("movements").select("*").eq("branch_id",bId).order("date",{ascending:false}).limit(300):supabase.from("movements").select("*").order("date",{ascending:false}).limit(300),
        supabase.from("suppliers").select("*").order("name"),
        supabase.from("branches").select("*").order("name"),
      ]);
      // Build stock map from branch_stock
      const stockMap={};(bs||[]).forEach(s=>{stockMap[s.pid]={qty:s.qty,min:s.min,bs_id:s.id};});
      // Merge products with branch stock
      const merged=(prods||[]).map(p=>({...p,qty:stockMap[p.id]?.qty??0,min:stockMap[p.id]?.min??p.min,bs_id:stockMap[p.id]?.bs_id}));
      setProducts(merged);setMovements(movs||[]);setSuppliers(sups||[]);setBranches(brs||[]);
      if(brs?.length&&!branch){const ub=brs.find(x=>x.id===user?.branch_id)||brs[0];setBranch(ub);}
  ...
  useEffect(()=>{
    ...
    const ch=supabase.channel("changes")
      .on("postgres_changes",{event:"*",schema:"public",table:"branch_stock"},()=>loadAll())
  ...
  // ── Upsert branch_stock helper ──────────────────────────────
  const upsertBS=async(bId,pid,newQty,existingBsId)=>{
    const bsId=existingBsId||"BS"+UID();
    await supabase.from("branch_stock").upsert({id:bsId,branch_id:bId,pid,qty:+(newQty).toFixed(4),updated_at:new Date().toISOString()},{onConflict:"branch_id,pid"});
    return bsId;
  };

  // ── Adjust (for stock count) ────────────────────────────────
  const handleAdjust=async(pid,pname,unit,cost,diffQty,note)=>{
    ...
    await Promise.all([supabase.from("movements").insert(mov),upsertBS(branch.id,pid,newQty,prod.bs_id)]);
  };
  ...
  const handleStockIn=async()=>{
    ...
    const[{error:e1}]=await Promise.all([supabase.from("movements").insert(mov),upsertBS(branch.id,prod.id,newQty,prod.bs_id)]);
    if(e1){setSiMsg("❌ เกิดข้อผิดพลาด");loadAll();}
    else{setSiForm(f=>({...f,pid:"",qty:"",cost:"",ref:"",note:""}));setSiMsg("✓ รับสินค้าเข้าเรียบร้อย");setTimeout(()=>setSiMsg(""),3500);}
    setOpLoad(false);
  };

  // ── Stock Out (รวม Transfer) ────────────────────────────────
  const handleStockOut=async()=>{
    ...
    if(soForm.type==="TRANSFER"&&!soForm.target_branch_id){setSoMsg("กรุณาเลือกสาขาปลายทาง");return;}
    setOpLoad(true);
    ...
    setMovements(m=>[mov,...m]);setProducts(ps=>ps.map(p=>p.id===prod.id?{...p,qty:newQty}:p));
    const promises=[supabase.from("movements").insert(mov),upsertBS(branch.id,prod.id,newQty,prod.bs_id)];
    // Transfer: also add stock to target branch
    if(soForm.type==="TRANSFER"&&soForm.target_branch_id){
      const{data:targetBS}=await supabase.from("branch_stock").select("*").eq("branch_id",soForm.target_branch_id).eq("pid",prod.id).single();
      const targetQty=+((targetBS?.qty||0)+qty).toFixed(4);
      const inMov={id:"M"+UID(),pid:prod.id,pname:prod.name,type:"STOCK_IN",qty,unit:prod.unit,cost:prod.cost,ref:`โอนจาก ${branch?.name}`,note:soForm.note||"",done_by:soForm.done_by||user.name,date:new Date(soForm.date).toISOString(),branch_id:soForm.target_branch_id};
      promises.push(supabase.from("movements").insert(inMov));
      promises.push(upsertBS(soForm.target_branch_id,prod.id,targetQty,targetBS?.id));
    }
    const results=await Promise.all(promises);
    const hasError=results.some(r=>r?.error);
    if(hasError){setSoMsg("❌ เกิดข้อผิดพลาด");loadAll();}
    else{
      setSoForm(f=>({...f,pid:"",qty:"",ref:"",note:"",target_branch_id:""}));
      setSoMsg(soForm.type==="TRANSFER"?"✓ โอนสต็อกเรียบร้อย ทั้งสองสาขาอัปเดตแล้ว":"✓ เบิกสินค้าออกเรียบร้อย");
  ...
  const handleSaveProduct=async(prod)=>{
    ...
    if(!error){
      if(isNew){
        // Create branch_stock (qty=0) for ALL branches automatically
        const entries=branches.map(b=>({id:"BS"+UID(),branch_id:b.id,pid:data.id,qty:0,min:data.min||0,updated_at:new Date().toISOString()}));
        if(entries.length>0)await supabase.from("branch_stock").insert(entries);
        setProducts(ps=>[...ps,{...data,qty:0}]);
      }else{setProducts(ps=>ps.map(p=>p.id===data.id?{...p,...data}:p));}
      setEditProduct(null);
  ...
  const renderContent=()=>{
    if(loading)return<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:300,gap:12,color:"#94a3b8"}}><RefreshCw size={20} style={{animation:"spin 1s linear infinite"}}/>กำลังโหลดข้อมูล...</div>;
    switch(tab){
      case"dashboard": return<Dashboard products={products} movements={movements} alerts={alerts} totalValue={totalValue} setTab={setTab} branch={branch}/>;
      case"products":  return<Products products={products} suppliers={suppliers} onEdit={setEditProduct} onAdd={()=>setEditProduct({sku:"",name:"",cat:CATS[0],unit:UNITS[0],cost:0,sell:0,sid:suppliers[0]?.id,min:0,loc:""})}/>;
      case"stockin":   return<StockIn products={products} form={siForm} setForm={setSiForm} onSubmit={handleStockIn} msg={siMsg} loading={opLoad}/>;
      case"stockout":  return<StockOut products={products} branches={branches} branch={branch} form={soForm} setForm={setSoForm} onSubmit={handleStockOut} msg={soMsg} loading={opLoad}/>;
      ...
      case"branches":  return<BranchesMgmt products={products} onCreateBranch={b=>setBranches(bs=>[...bs,b])}/>;
        ...
        <div style={{width:210,background:"#0c1929",display:"flex",flexDirection:"column",flexShrink:0}}>
          <div style={{padding:"20px 18px 16px",borderBottom:"1px solid #1a2d42"}}>
            <div style={{display:"flex",alignItems:"center",gap:9}}>
              ...
              <div><div style={{color:"#f0f9ff",fontWeight:800,fontSize:14}}>STOCK PRO</div><div style={{color:"#4a6380",fontSize:10}}>Multi-Branch</div></div>
            </div>
          </div>
          {/* User */}
          <div style={{padding:"12px 16px",borderBottom:"1px solid #1a2d42",background:"#0a1520"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{width:28,height:28,borderRadius:"50%",background:ROLE_COLOR[user.role]+"30",display:"flex",alignItems:"center",justifyContent:"center",color:ROLE_COLOR[user.role],fontSize:12,fontWeight:700}}>{user.name.charAt(0)}</div>
          ...
          <div style={{padding:"8px 12px",borderBottom:"1px solid #1a2d42"}}>
            <div style={{fontSize:10,color:"#4a6380",marginBottom:4,fontWeight:600}}>📍 สาขาที่ดูอยู่</div>
            <select value={branch?.id||""} onChange={e=>{const b=branches.find(x=>x.id===e.target.value);setBranch(b);loadAll(b?.id);}} style={{...S,background:"#1a2d42",color:"#94a3b8",border:"1px solid #2d4a65",fontSize:12}}>
          ...
          <div style={{padding:"10px 16px",borderTop:"1px solid #1a2d42"}}>
            <button onClick={()=>loadAll()} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"7px",background:"#1a2d42",color:"#6b8aaa",border:"none",borderRadius:8,cursor:"pointer",fontSize:12,fontFamily:"inherit"}}><RefreshCw size={12}/>รีเฟรช</button>
            <div style={{marginTop:8,fontSize:11,color:"#4a6380",lineHeight:1.9}}>
              ...
              <div>สาขา: <span style={{color:"#7ea8c8"}}>{branches.length} สาขา</span></div>
            </div>
          </div>
        </div>
        {/* Main */}
